...
export default function ProductModal({ isOpen, onClose, product, onComplete }: Props) {
  ...
  useEffect(() => {
    fetch('/api/categories', { headers: { 'Authorization': `Bearer ${localStorage.getItem('whiz-token')}` } })
  ...
  useEffect(() => {
    if (product) {
      ...
      setFormData({
        sku: '', name: '', categoryId: '', price: 0, costPrice: 0, stock: 0, reorderLevel: 5
      });
    }
  }, [product, isOpen]);
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="mb-4">
            ...
            <select className="w-full border border-gray-300 rounded-lg px-4 py-2" value={formData.categoryId} onChange={e => setFormData({ ...formData, categoryId: e.target.value })}>
