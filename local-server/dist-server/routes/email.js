...
const getEmailTransporter = async (businessId) => {
...
// Send a receipt
router.post('/send-receipt', authenticate, async (req, res) => {
    try {
        const { businessId } = req.user;
        ...
        const transaction = await db.selectFrom('Transaction').selectAll().where('id', '=', receiptId).where('businessId', '=', businessId).executeTakeFirst();
        if (!transaction)
            return res.status(404).json({ error: 'Transaction not found' });
        // Try to find the cashier name
        let cashierName = 'Cashier';
        if (transaction.userId) {
            const user = await db.selectFrom('User').select('name').where('id', '=', transaction.userId).executeTakeFirst();
            if (user)
                cashierName = user.name;
        }
        // Try to get receipt items
        const items = await db.selectFrom('ReceiptItem').selectAll().where('transactionId', '=', receiptId).execute();
...
router.post('/send-custom', authenticate, async (req, res) => {
    try {
        ...
        const fullHtml = generateEmailHtml(businessName, logoUrl, htmlContent, settings.emailFrom);
        const mailOptions = {
            from: `"${businessName}" <${settings.emailFrom}>`,
            to,
            replyTo: settings.emailReplyTo || settings.emailFrom,
